...
if EEG.trials > 1
    ...
    [dum,dum,dum,dum,hndl] = erpimage( icaacttmp-offset, ones(1,EEG.trials)*10000, EEG.times*1000, ...
        '', ei_smooth, 1, 'caxis', 2/3, 'cbar','erp', 'yerplabel', '','erp_vltg_ticks',era_limits);
    command = [ 'p = getpointerpos(gca);[dum,time] = timepts(p(1));tri = round(p(2));clear p;'...
        'tmpEEG = pop_subcomp(EEG,' num2str(chanorcomp) ');'...
        'figure(4848);clf;'...
        'plottopo(cat(3,EEG.data(EEG.icachansind,:,tri),tmpEEG.data(EEG.icachansind,:,tri)),''chanlocs'',EEG.chanlocs,''frames'',' num2str(size(EEG.data,2)) ');'...
        ];
    set(findobj(hndl{1},'type','image'),'buttondownfcn',command);
...
%     % plotsubcomp button
%     % ---------
%     command = [ 'tmpEEG = pop_subcomp(' num2str(chanorcomp) ');'...
%                 'd = EEG.data - tmpEEG.data;'...
%                 '[m,i] = max(d(:)); [chan, time, tri] = ind2sub([EEG.nbchan EEG.pnts, EEG.trials],i);'...
%                 'figure(4848);clf;'...
%                 'plot(EEG.times,EEG.data(chan,:,tri),EEG.times,tmpEEG.data(chan,:,tri));'...
%                 'ylim([-m,m]);'...
%                 
%         ];
%     if winhandle ~= 0
...
%     end;
%     command = [ command 'close(gcf); clear tmpstatus' ];
%     h  = uicontrol(gcf, 'Style', 'pushbutton', 'string', 'w/wo', 'backgroundcolor', GUIBUTTONCOLOR, 'Units','Normalized', 'Position',[90 -10 15 6].*s+q, 'callback', command);
